...
function getDestinationCertificate(destinationLookUpResult){
  if (!destinationLookUpResult.certificates){
    return null;
  }
  const { KeyStoreLocation, KeyStorePassword ,TrustStoreLocation } = destinationLookUpResult.destinationConfiguration ;
  const { certificates } =  destinationLookUpResult ;
  const certificatesResult = [];
  certificates.forEach((certificate) => {
    const {  Content , Name } = certificate;
    const extension = Name.split('.').pop();
    if (Name === KeyStoreLocation){
      if (extension !== 'p12'){
        throw new Error('not supported certificate format');
      }
...
function getDestinationInstanceCredentialsByTenant(req, cb) {
  ...
  exports.getDestinationsByTenant(null, token,req.zoneInfo, function(err, destinations) {
    if (err) {
      return cb(err);
    }
