...
async function createSessionByAuthCode(req,cb){
  try {
    ...
    const oidcToken  = await exports.fetchServiceToken(req.query.approuterAuthCode);
...
async function fetchServiceToken(approuterAuthCode) {
  ...
  if (credentials){
    const tokenInfo = await tokenUtils.getClientCredentialsToken(null, credentials.uaa, null);
...
function exchangeOIDCTokenByXSUAAToken(req, oidcToken,uaaOptions){
  return new Promise((resolve,reject) => {
    tokenUtils.exchangeToken(oidcToken, uaaOptions,(err, accessToken, xsuaaToken) => {
      if (err) {
